...
class AnyTextMixin:
    ...
    def test_with_conflicts(self):
        ...
        self.assertEqual([(1, "File has conflicts.")], self.reporter.messages)
    ...
    def test_long_length(self):
        ...
        self.assertEqual([(1, "Line exceeds 80 characters.")], self.reporter.messages)
    ...
    def test_with_tabs(self):
        ...
        self.assertEqual(
            [(1, "Line contains a tab character.")], self.reporter.messages
        )
    ...
    def test_regex_line(self):
        ...
        self.create_and_check("bogus", "with marker here\n other sign here", options)
...
class TestText(CheckerTestCase, AnyTextMixin):
    ...
    def create_and_check(self, file_name, text, options=None):
        checker = AnyTextChecker(file_name, text, self.reporter, options)
        checker.check()
    ...
    def test_long_length_options(self):
        ...
        self.assertEqual([(1, "Line exceeds 49 characters.")], self.reporter.messages)
    ...
    def test_windows_newlines(self):
        ...
        checker.check()
        self.assertEqual(
            [(0, "File contains Windows new lines.")], self.reporter.messages
        )
    ...
    def test_single_last_line_no_newline(self):
        ...
        content = "the second and last line without newline"
        checker = AnyTextChecker("bogus", content, self.reporter)
        checker.check_empty_last_line(1)
        expected = [(1, "File does not ends with an empty line.")]
        self.assertEqual(expected, self.reporter.messages)
        self.assertEqual(1, self.reporter.call_count)
